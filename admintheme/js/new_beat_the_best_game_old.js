$( "#valid" ).validationEngine( 'attach' , {
	onValidationComplete : function( form , status )
	{
		...
		if (status == true) {
			if(no_of_games < 1 && entry_fee == 0)
			{
				jAlert ( 'Min 1 Games should be selected' );
				return false;
			}
			else if(no_of_games < 2 && entry_fee != 0)
			{
...
$.extend( NewGame.prototype , {
	...
	entry_fee_combo					: 'Select Entry Fee',
	...
	ShowWeekDates : function ( obj )
	{
		...
		else
		{
			...
			else if ( duration_id == 2 ) //Weekly MLB
			{
				newgameobj.ResetCombo( 'buckets' );
			...
			success		: function( response )
			{
				...
				if(duration_id == 1)
				{
					$.each( list , function ( key , value ){
						...
						{
							$( '.game_list' ).append( '<div class="show_week_list"><input type="checkbox" checked class="checkbox_sel" value='+value.season_game_unique_id+'  onchange="new_game.list_of_game(this)" id="my_'+value.season_game_unique_id+'" ><span>'+value.home+'@'+value.away+' '+value.season_scheduled_date+' ('+value.day_name+') </span></input></div>' );
						}else
						{
							$( '.game_list' ).append( '<div class="show_week_list"><input type="checkbox" class="checkbox_sel" value='+value.season_game_unique_id+'  onchange="new_game.list_of_game(this)" id="my_'+value.season_game_unique_id+'" ><span>'+value.home+'@'+value.away+' '+value.season_scheduled_date+' ('+value.day_name+') </span></input></div>' );
						}
					});
				}
				else if(duration_id == 2)
				{
					$.each( list , function ( key , value ){
						if($.inArray(value.season_game_unique_id,game_selected) != -1)
						{
							$( '.insert_weekly_games' ).append( '<div class="show_week_list"><input type="checkbox" checked class="checkbox_sel" value='+value.season_game_unique_id+'  onchange="new_game.list_of_game(this)" id="my_'+value.season_game_unique_id+'" ><span>'+value.home+'@'+value.away+' '+value.season_scheduled_date+' ('+value.day_name+') </span></input></div>' );
						}else
						{
							$( '.insert_weekly_games' ).append( '<div class="show_week_list"><input type="checkbox" class="checkbox_sel"  value='+value.season_game_unique_id+'  onchange="new_game.list_of_game(this)" id="my_'+value.season_game_unique_id+'" ><span>'+value.home+'@'+value.away+' '+value.season_scheduled_date+' ('+value.day_name+') </span></input></div>' );
						}
					});
				}
			}
	...
	list_of_game : function(ths,val)
	{
		...
		//If daily duration is selected then only two games can be selected
		if(duration_id == 1)
		{	
			if(isChecked == true)
			{	
		...
		//If weekly duration is selected then only unlimited games can be selected
		else if(duration_id == 2)
		{
			if(isChecked == true)
			{	
	...
	CreateGameName : function()
	{
		...
		{
			league_salary_cap_label = league_salary_cap_label.replace( /0+/ ,'' ) + '00k';
		}
		...
		var game_name = league_label+' '+league_duration_label+' '+date_label;
		$( '#game_name' ).val( '' );
...
$( '#buckets' ).change(function (argument) {
...
$( '#season_week_id' ).change(function (argument) {
...
$(document).ready(function() {
    $('#selectall,#selectall_daily').click(function(event) {  //on click 
    	...
    	//console.log($('.selected_games').length);
