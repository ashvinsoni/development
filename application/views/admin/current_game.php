...
<div class="content">
	<div class="title">
		<h5>
			<?php 
				$list_title = explode('_', $list_for);				
				for ($i=0; $i < count($list_title); $i++) 
				{ 
					echo ucfirst($list_title[$i])." ";
				}
	...
	<?php $frm_attr = 'id="game_list" class="frm_input"'; echo form_open( current_url() , $frm_attr ); ?>
		<div class="stats slect-league">
			<ul>
				<li>
					<?php $attr = ' onchange="Games.SearchGame();" id="league_id_drp"'; if ( isset( $sport) ) echo form_dropdown( 'league_id' , $sport ,'' , $attr ); ?>
				</li>
			</ul>
			<div class="fix"></div>
		</div>
		...
		<div class="pagination">
			...
			<div class="per_page">
				<?php if ( isset( $per_page_limit ) ){ $attr = 'id="per_page_limit" onchange="Games.SearchGame();"'; echo form_dropdown( 'per_page_limit' , $per_page_limit , '' , $attr );  } ?>
				<label for="per_page_limit" style="float:right;margin-right:5px;">Records Per Page:</label>
			</div>
		</div>
		<input type="hidden" name="list_for" value="<?php echo $list_for; ?>" />
	<?php echo form_close(); ?>
</div>
...
<script type="text/x-handlebars-template" id="GameListingTemplete">
	{{#each this}}
		<tr>
			<?php /*<td align="center">{{this.serial_no}}</td>*/ ?>
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#league_id_drp ").prepend("<option value='' selected='selected'>Select League</option>").change();
		// Games.SearchGame();
	});
</script>